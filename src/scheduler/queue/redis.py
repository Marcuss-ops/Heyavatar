...
The ``redis`` package is imported lazily so the rest of the
dependency tree stays usable in dev / CI environments without a
Redis server installed. The class also exposes
:meth:`_reclaim` / :meth:`_trim` housekeeping that runs once per
process lifetime to keep the stream bounded.
"""
