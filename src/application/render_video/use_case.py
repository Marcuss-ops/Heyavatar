"""RenderVideo use case — orchestrator for long-form chunked rendering.

Drives an engine through a long-form render in 3 phases:

1. **Chunking** — split the audio into overlapping chunks via
   :meth:`RenderVideo._chunks_for`.
2. **Per-chunk render** — render each chunk with retry/backoff via
   :meth:`RenderVideo._render_one_chunk`, publishing per-chunk GPU
   telemetry so partial-job costs survive a later failure.
3. **Manifest** — write a chunk-list manifest for the
   :class:`EncodingWorker` to assemble the final video.

GPU telemetry is published per-chunk so partial-job costs are visible.
"""
...
from src.application.render_video import audio_probe
...
class RenderVideo:
    ...
    def _chunks_for(self, request: RenderRequest) -> Iterable[RenderChunkRequest]:
        cfg = self.chunk_config
        audio_duration = audio_probe._probe_audio_duration(request.render_spec.audio_path)
        if audio_duration <= 0:
