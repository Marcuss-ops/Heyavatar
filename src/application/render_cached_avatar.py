...
def render_cached_avatar(
    ...
    *,
    ...
    body = body_template_loader(avatar_id, gesture_id, base_dir=body_templates_dir)
    body_cache_hit = True
    LOG.info(
        "render_cached_avatar: avatar_id=%s gesture_id=%s body_dir=%s",
        avatar_id, gesture_id, body.body_video.parent,
    )
    ...
    try:
        ...
        LOG.warning("mux_audio failed (%s); running QC on composited.mp4", exc)
        final_path = None
        qc_video_path = composited_path
    ...
    _write_metrics(capture_root / job_id, metrics, qc_result, output_path)
    return result
