...
def main():
    ...
    engine.load()
    # Keep the avatar alive but anchored: subtle speech motion, no gaze drift.
    engine.inf_cfg.extra["static_head"] = False
    engine.inf_cfg.extra["motion_style"] = "subtle"
    engine.inf_cfg.extra["motion_intensity"] = 0.9
    engine.inf_cfg.extra["eye_lock"] = True
    ...
    try:
        ...
        job_id_crop = RenderJobId("user_avatar_crop_presenter")
        ...
        # 3. Render 1080p Video (face_region_only = False, steady presenter motion)
        print("\nRendering Full 1080p Presenter Motion Video...")
        job_id_full = RenderJobId("user_avatar_1080p_presenter")
        ...
        print(f"-> Saved expressive motion video to: {final_full_path}")
