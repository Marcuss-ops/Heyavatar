...
def _do_process_impl(self, job: RenderJob, engine, repo: AvatarPackRepository):
    ...
    * ``render``  — pull the identity, render audio chunks, hand the
    ...
    requested_motion_style = payload.get("motion_style")
    original_engine_settings = getattr(engine, "settings", None)
    if requested_motion_style and original_engine_settings is not None:
        try:
            engine.settings = replace(original_engine_settings, motion_style=str(requested_motion_style))
        except Exception:
            pass
    try:
        # ── compile-only job ──────────────────────────────────────
        if job_type == "compile":
            ...
            return JobState.COMPLETED, {
                ...
                "motion_style": requested_motion_style,
            }
        ...
        if payload.get("text"):
            ...
            output_dir = self.settings.capture_dir / str(job.id)
            output_dir.mkdir(parents=True, exist_ok=True)
            output_path = output_dir / "final.mp4"
            body_dir = getattr(self.settings, "body_templates_dir", "body_templates")
            result = run_cached_avatar_from_text(
                text=payload["text"],
                audio_path=Path(payload["audio_path"]),
                output_path=output_path,
                avatar_id=identity_id_str,
                engine=engine,
            ...
            degraded = len(result.skipped_gestures) > 0
            render_state = JobState.COMPLETED_DEGRADED if degraded else JobState.COMPLETED
            gpu_seconds_total = sum(
            ...
            return render_state, {
                ...
                "duration_seconds": result.render_seconds_total,
                "gpu_seconds": gpu_seconds_total,
                "degraded": degraded,
                "degraded_chunks": list(result.skipped_gestures),
                "total_chunks": result.segment_count,
                "motion_style": requested_motion_style,
            }
        ...
        if original_engine_settings is not None and requested_motion_style:
            try:
