...
class MuseTalkAdapter(AvatarEngine):
    ...
    def prepare_identity(self, source_image: Path) -> Dict[str, bytes]:
        ...
        * ``alignment_matrix.bin`` — affine alignment matrix (float32).
        ...
        # Real-mode: never substitute mock assets. If the engine could
        # not load (missing checkpoint, CUDA absent, upstream package
        # importable=false), raise so the caller routes to FAILED_INFERENCE.
